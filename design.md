...
### Extension tokens (interactive sims)

Physics/chemistry sims need a handful of semantically distinct plot colours. Don't invent a new palette — map them onto the existing semantic tokens so nothing off-brand (neon cyan, violet) leaks in:

```css
:root {
  --color-vb:      #274FE3;   /* --info    — the quantity the student controls */
  --color-vr:      #EA580C;   /* --warn    — the opposing / environmental quantity */
  --color-vR:      #1CAB55;   /* --success — the resulting answer */
  --color-warning: #E8001D;   /* --ten-red — impossible / error state */

  --tint-vR:  rgba(28, 171, 85, 0.10);
  --tint-red: rgba(232, 0, 29, 0.10);
}
```

Rule of thumb: **blue for input, orange for opposition, green for the answer, red for "this can't happen."**

### Token rules

- Body text is `var(--ten-ink)` (`#111827`), **not** `#000`.
- **Write theme-varying tokens as literals, not as `var()` of another token.** A dark-mode block that redefines `--surface-card` will not reliably reach `<button>` elements if the light value was `var(--ten-white)` — Chrome keeps the stale value. Literals in both blocks, with the source token named in a comment.
- **Suppress transitions for the frame in which a theme flips.** A whole-page colour cross-fade is not a 10MS motion pattern, and a running transition otherwise pins controls to the previous palette. Add `html.theme-switching *, ::before, ::after { transition: none !important; }`, apply it, flip the class, force a reflow, remove it.
...
### 8.14 Simulation controls (TenTen sims)

Sims are touch-first (see `implementation-guide.md` §4.2.1), so their controls are built taller than the equivalent desktop widget.

**Range slider** — the visible track stays 6px, but the input itself claims a 44px touch band:

```css
.range-slider {
  -webkit-appearance: none; appearance: none;
  width: 100%; height: 44px;        /* touch band, not the visual height */
  background: transparent; cursor: pointer;
}
.range-slider::-webkit-slider-runnable-track {
  height: 6px; border-radius: var(--r-pill); background: var(--gray-200);
}
.range-slider::-webkit-slider-thumb {
  -webkit-appearance: none; appearance: none;
  width: 24px; height: 24px; margin-top: -9px; border-radius: 50%;
  background: var(--ten-ink); border: 3px solid var(--surface-card);
  box-shadow: var(--sh-card);
}
```

Colour each thumb with the quantity it controls (`--color-vb`, `--color-vr`, …) so the slider, the swatch in its label, and the plotted vector all agree.

**Persistent selector description** — a group tab's explanation goes in a panel below the tab strip, never in a `:hover` tooltip. Touch devices cannot hover, and a sim that hides its instructions behind hover is broken on the platform most students use.
...
**Canvas text** — a `<canvas>` cannot read CSS custom properties. Resolve them in JS (`getComputedStyle(document.documentElement).getPropertyValue('--color-vR')`) rather than passing `var(--x)` to `ctx.fillStyle`, where it silently does nothing. Canvas fonts must name a loaded family: `'bold 9px Inter, "Hind Siliguri", sans-serif'`.
...
| **TenTen sims** (embedded simulations) | Light, mobile-first | Green | Group tabs + persistent description, 44px touch sliders, canvas/SVG plots, guided tutorial overlay |
